...
std::string replace_value(std::string_view xml, std::string_view tag,
...
std::string insert_after_root_open(std::string_view xml,
                                   std::string_view markup) {
  std::string result{xml};
  const auto position =
      result.find("<netft>") + std::string_view{"<netft>"}.size();
...
public:
  FloatingPointEnvironmentGuard()
      : saved_{}, valid_{std::fegetenv(&saved_) == 0} {}
  ...
  FloatingPointEnvironmentGuard &
  operator=(const FloatingPointEnvironmentGuard &) = delete;
...
TEST(SensorDiscovery, RejectsInvalidPublicOptionsBeforeNetworkIo) {
  const std::vector<std::function<void(netft::DiscoveryOptions &)>> cases{
      ...
      [](auto &options) {
        options.connect_timeout = std::chrono::duration<double>{0.0};
      },
      [](auto &options) {
        options.connect_timeout = std::chrono::duration<double>{-1.0};
      },
      [](auto &options) {
        options.connect_timeout = std::chrono::duration<double>{
      ...
      [](auto &options) {
        options.total_timeout = std::chrono::duration<double>{0.0};
      },
      [](auto &options) {
        options.total_timeout = std::chrono::duration<double>{-1.0};
      },
      [](auto &options) {
        options.total_timeout = std::chrono::duration<double>{
            std::numeric_limits<double>::infinity()};
      },
      [](auto &options) {
        options.total_timeout = std::chrono::duration<double>{
...
INSTANTIATE_TEST_SUITE_P(EveryField, RequiredXmlField,
                         ::testing::Values("prodname", "cfgcpf", "cfgcpt",
                                           "scfgfu", "scfgtu"));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        InvalidCountCase{"cfgcpf", "not-a-number"},
        InvalidCountCase{"cfgcpf", "10remaining"},
        InvalidCountCase{"cfgcpf", "0"}, InvalidCountCase{"cfgcpf", "-1"},
        InvalidCountCase{"cfgcpf", "+1"}, InvalidCountCase{"cfgcpf", "NaN"},
        InvalidCountCase{"cfgcpf", "inf"}, InvalidCountCase{"cfgcpf", "1e9999"},
        InvalidCountCase{"cfgcpf", "1e-9999"},
        InvalidCountCase{"cfgcpt", "not-a-number"},
        InvalidCountCase{"cfgcpt", "10remaining"},
        InvalidCountCase{"cfgcpt", "0"}, InvalidCountCase{"cfgcpt", "-1"},
        InvalidCountCase{"cfgcpt", "NaN"},
...
TEST(XmlConfiguration, ParsesCountsIndependentlyOfTheGlobalLocale) {
  ...
  EXPECT_DOUBLE_EQ(result.calibration.counts_per_force_unit, 1234.5);
  EXPECT_THROW(netft::detail::parse_sensor_configuration(
                   replace_value(kValidXml, "cfgcpf", "1234,5")),
               netft::DiscoveryError);
}
...
TEST(XmlConfiguration, UsesRoundToNearestAndRestoresTheCallersRoundingMode) {
  ...
  const auto xml =
      replace_value(kValidXml, "cfgcpf",
...
TEST(XmlConfiguration, RestoresTheCallersRoundingModeWhenParsingThrows) {
  ...
  EXPECT_THROW(netft::detail::parse_sensor_configuration(
                   replace_value(kValidXml, "cfgcpf", "invalid")),
               netft::DiscoveryError);
  EXPECT_EQ(std::fegetround(), FE_UPWARD);
}
...
class FloatingPointEnvironment
    : public ::testing::TestWithParam<FloatingPointEnvironmentCase> {};
...
TEST_P(FloatingPointEnvironment, PreservesTheCallersModeAndExceptionFlags) {
  ...
  if (GetParam().succeeds) {
    ...
    EXPECT_THROW(netft::detail::parse_sensor_configuration(xml),
                 netft::DiscoveryError);
  }
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        FloatingPointEnvironmentCase{
            FE_TONEAREST,
            "1.00000000000000011102230246251565404236316680908203125", true},
...
#if defined(__linux__) && defined(__GLIBC__)
TEST(FloatingPointTrapPlatformPolicy,
     TreatsUnavailableTrapControlAsAnErrorWhereItIsSupported) {
#if defined(__aarch64__)
...
TEST(XmlConfiguration, MasksCallerOverflowTrapsWhileParsing) {
  ...
  if (child == 0) {
    ...
    try {
      static_cast<void>(netft::detail::parse_sensor_configuration(
          replace_value(kValidXml, "cfgcpf", "1e9999")));
  ...
  if (WEXITSTATUS(status) == kTrapControlUnsupported) {
    GTEST_SKIP()
        << "floating-point trap control is unavailable on this platform";
  }
...
TEST(XmlConfiguration, AcceptsEveryApprovedForceUnitSpelling) {
  ...
  for (const auto &[spelling, expected] : cases) {
    ...
    const auto result = netft::detail::parse_sensor_configuration(
        replace_value(kValidXml, "scfgfu", spelling));
...
TEST(XmlConfiguration, AcceptsEveryApprovedAtiTorqueUnitSpelling) {
  ...
  for (const auto &[spelling, expected] : cases) {
    SCOPED_TRACE(spelling);
    const auto result = netft::detail::parse_sensor_configuration(
        replace_value(kValidXml, "scfgtu", spelling));
...
TEST(XmlConfiguration, RejectsUnknownUnits) {
  EXPECT_THROW(netft::detail::parse_sensor_configuration(
                   replace_value(kValidXml, "scfgfu", "newtons")),
               netft::DiscoveryError);
  EXPECT_THROW(netft::detail::parse_sensor_configuration(
...
TEST(XmlConfiguration, IgnoresRequiredElementTextInsideComments) {
  const auto xml = insert_after_root_open(
      kValidXml, "<!-- <prodname>Fake Sensor</prodname> -->");
...
TEST(XmlConfiguration, RejectsAFieldThatAppearsOnlyInsideAComment) {
  const auto xml =
      insert_after_root_open(remove_element(kValidXml, "prodname"),
...
TEST(XmlConfiguration, IgnoresRequiredElementTextInsideCdata) {
  const auto xml = insert_after_root_open(
      kValidXml, "<![CDATA[<prodname>Fake Sensor</prodname>]]>");
...
TEST(XmlConfiguration, RejectsUnterminatedCommentsAndCdata) {
  EXPECT_THROW(netft::detail::parse_sensor_configuration(
                   std::string{kValidXml} + "<!-- unterminated"),
               netft::DiscoveryError);
  EXPECT_THROW(netft::detail::parse_sensor_configuration(
                   std::string{kValidXml} + "<![CDATA[unterminated"),
               netft::DiscoveryError);
}

TEST(XmlConfiguration, RejectsOverlappingElementNesting) {
  const auto xml =
      replace_value(kValidXml, "prodname", "Ethernet Axia</wrapper>");
  const auto malformed = std::string{"<wrapper>"} + xml;

  EXPECT_THROW(netft::detail::parse_sensor_configuration(malformed),
               netft::DiscoveryError);
}
...
TEST(SensorDiscovery, RejectsRedirectResponses) {
  ...
  redirecting_server.set_redirect_location(
      "http://" + redirect_target.host() + ":" +
      std::to_string(redirect_target.port()) + "/netftapi2.xml");

  EXPECT_THROW(netft::discover_sensor(options_for(redirecting_server)),
               netft::DiscoveryError);
...
TEST(SensorDiscovery, RejectsAResponseLargerThan64KiB) {
  ...
  EXPECT_THROW(netft::discover_sensor(options_for(server)),
               netft::DiscoveryError);
}
...
TEST(SensorDiscovery, RejectsHostsThatInjectUrlSyntax) {
  ...
  constexpr std::array<std::string_view, 4> malicious_hosts{
      "ignored-userinfo@127.0.0.1", "127.0.0.1/ignored", "127.0.0.1?ignored",
      "127.0.0.1#ignored"};
...
TEST(SensorDiscovery, RejectsRoundedMillisecondsAboveLongMax) {
  ...
  options.connect_timeout = std::chrono::milliseconds{50};
  options.total_timeout = std::chrono::duration<double>{
      static_cast<double>(std::numeric_limits<long>::max()) / 1000.0};
...
TEST(SensorDiscovery, AcceptsRoundedMillisecondsAtOrBelowLongMax) {
  ...
  options.total_timeout =
      std::chrono::duration<double>{std::nextafter(boundary_seconds, 0.0)};
...
TEST(FakeHttpServerTest, DestructionInterruptsAnIncompleteAcceptedRequest) {
  ...
  ASSERT_EQ(::connect(client, reinterpret_cast<const sockaddr *>(&address),
            ...
            0);
  ASSERT_EQ(netft::test::send_socket(client, "GET /netftapi2.xml", 19,
                                     netft::test::socket_send_flags()),
            19);

  for (int attempt = 0;
       attempt < 100 && server->accepted_connection_count() == 0; ++attempt) {
