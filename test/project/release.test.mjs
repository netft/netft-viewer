...
test("release metadata agrees on the package version", async () => {
  const pkg = JSON.parse(await load("package.json"));
  const cmake = await load("CMakeLists.txt");
  const pixi = await load("pixi.toml");
  const changelog = await load("CHANGELOG.md");
  const version = escapeRegularExpression(pkg.version);

  assert.match(
    cmake,
    new RegExp(`project\\(netft_viewer VERSION ${version}\\b`),
  );
  assert.match(pixi, new RegExp(`^version = "${version}"$`, "m"));
  assert.match(
    changelog,
...
test("tag releases reuse every package target and produce integrity metadata", async () => {
  const release = parse(await load(".github/workflows/release.yml"), {
    uniqueKeys: true,
  });
  const packaging = parse(await load(".github/workflows/package.yml"), {
    uniqueKeys: true,
  });

  assert.deepEqual(release.on.push.tags, ["v*"]);
  assert.equal(
    Object.prototype.hasOwnProperty.call(packaging.on, "workflow_call"),
    true,
  );
  assert.equal(packaging.on.push.tags, undefined);
  assert.equal(release.jobs.packages.uses, "./.github/workflows/package.yml");

  const packageTargets = new Set(
    packaging.jobs.package.strategy.matrix.include.map(
      ({ platform, architecture }) => `${platform}/${architecture}`,
    ),
  );
  assert.deepEqual(
    packageTargets,
    new Set(["linux/x64", "linux/arm64", "win32/x64", "darwin/universal"]),
  );

  const prepareSteps = new Map(
    release.jobs.prepare.steps.map((step) => [step.id, step]),
  );
  assert.equal(
    prepareSteps.get("download-linux").uses,
  ...
  assert.equal(
    ...
    "netft-viewer-macos-universal",
  );
  assert.match(
    prepareSteps.get("download-macos-unsigned").if,
    /MACOS_SIGNING_ENABLED != 'true'/,
  );
  assert.equal(
    prepareSteps.get("download-macos-signed").with.name,
    "netft-viewer-macos-universal-signed",
  );
  assert.match(
    prepareSteps.get("download-macos-signed").if,
    /MACOS_SIGNING_ENABLED == 'true'/,
  );
  assert.match(prepareSteps.get("checksums").run, /SHA256SUMS/);
  assert.equal(
    prepareSteps.get("sbom").uses,
    "anchore/sbom-action@e22c389904149dbc22b58101806040fa8d37a610",
  );
  assert.equal(prepareSteps.get("sbom").with["syft-version"], "v1.44.0");
  assert.equal(prepareSteps.get("sbom").with.format, "spdx-json");
  assert.match(prepareSteps.get("verify").run, /cmp\b/);
});

test("release creation is draft-first and publication is protected", async () => {
  const workflow = parse(await load(".github/workflows/release.yml"), {
    uniqueKeys: true,
  });
  const publisher = await load(".github/scripts/publish_release.sh");

  assert.equal(workflow.permissions.contents, "read");
  assert.equal(workflow.jobs.draft.needs, "prepare");
  assert.equal(workflow.jobs.draft.permissions.contents, "write");
  assert.equal(workflow.jobs.publish.needs, "draft");
  assert.equal(workflow.jobs.publish.environment, "release");
  assert.equal(workflow.jobs.publish.permissions.contents, "write");
  assert.match(
    workflow.jobs.draft.steps.at(-1).run,
    /publish_release\.sh stage/,
  );
  assert.match(
    workflow.jobs.publish.steps.at(-1).run,
    /publish_release\.sh publish/,
  );
  assert.match(publisher, /gh release create[\s\S]*--verify-tag/);
});
