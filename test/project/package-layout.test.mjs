...
test("ASAR entry paths use portable separators", async () => {
  const { normalizeArchivePaths } =
...
test("canonical path comparison follows target platform semantics", async () => {
  const { canonicalPathsEqual } =
    await import("../../tools/lib/package-layout.mjs");
  assert.equal(
    canonicalPathsEqual(
      "C:\\Package\\Resources\\LICENSE",
      "c:\\package\\resources\\license",
      "win32",
    ),
    true,
  ...
  assert.equal(
    canonicalPathsEqual("/Package/LICENSE", "/package/license", "linux"),
    false,
  );
  assert.equal(
    canonicalPathsEqual(
...
test("regular package files allow only aliases above the trusted root", async (context) => {
  const { requireRegularPackageFile } =
    await import("../../tools/lib/package-layout.mjs");
  const temporary = await mkdtemp(join(tmpdir(), "netft-viewer-files-"));
  context.after(() => rm(temporary, { force: true, recursive: true }));
  const root = join(temporary, "package");
  const outside = join(temporary, "outside");
  const rootAlias = join(temporary, "package-alias");
  await mkdir(root, { recursive: true });
  await mkdir(outside, { recursive: true });
  await writeFile(join(root, "regular"), "data");
  await writeFile(join(root, "empty"), "");
  await writeFile(join(outside, "external"), "data");
  await symlink(root, rootAlias, "dir");
  await symlink(outside, join(root, "linked-directory"), "dir");
  await symlink(join(outside, "external"), join(root, "linked-file"));

  assert.equal(
    await requireRegularPackageFile(rootAlias, "regular"),
    join(rootAlias, "regular"),
  );
  await assert.rejects(
    requireRegularPackageFile(root, "linked-directory/external"),
  );
  await assert.rejects(requireRegularPackageFile(root, "linked-file"));
  await assert.rejects(requireRegularPackageFile(root, "empty"));
  await assert.rejects(requireRegularPackageFile(root, "../outside/external"));
});
...
test("distributed legal resources are regular, complete, and byte-identical", async (context) => {
  ...
  context.after(() => rm(temporary, { force: true, recursive: true }));
  const aliasedRoot = `${temporary}-alias`;
  context.after(() => rm(aliasedRoot, { force: true, recursive: true }));
  ...
  await symlink(temporary, aliasedRoot, "dir");
  await verifyDistributedNotices(aliasedRoot);
