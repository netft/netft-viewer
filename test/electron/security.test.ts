...
describe("Electron window security", () => {
  it("constructs the actual viewer window with an isolated sandbox", async () => {
    await createViewerWindow({
      ...
      iconPath: resolve("packaging/icons/netft-viewer.png"),
  ...
  it("installs native integrations before renderer startup can publish state", async () => {
    const order: string[] = [];
    const beforeLoad = vi.fn(() => {
      order.push("integration");
    });
    class OrderedWindow extends FakeBrowserWindow {
      override readonly loadURL = vi.fn(async (url: string) => {
        order.push("renderer");
        this.webContents.mainFrame.url = url;
      });
    }
    ...
    expect(order).toEqual(["integration", "renderer"]);
  });
  ...
  it("authorizes the React refresh preamble without allowing arbitrary inline scripts", async () => {
    const session = fakeSession();
    installSessionSecurity(session, true);
    ...
    const digest = createHash("sha256").update(preamble).digest("base64");
    const scriptPolicy =
      developmentPolicy
        .split("; ")
  ...
  it("keeps the Linux desktop identity aligned across runtime and package metadata", async () => {
    const packageMetadata = JSON.parse(
      await readFile(resolve("package.json"), "utf8"),
    ) as { desktopName: string; name: string };
    const debMaker = forgeConfig.makers?.find(
      (maker) => typeof maker === "object" && maker?.name === "deb",
    ) as
      | {
          configOrConfigFetcher?: {
            options?: { desktopTemplate?: string; name?: string };
          };
        }
      | undefined;
    const debOptions = debMaker?.configOrConfigFetcher?.options;
    const templatePath = debOptions?.desktopTemplate;

    expect(packageMetadata.desktopName).toBe(`${packageMetadata.name}.desktop`);
    expect(templatePath).toBeDefined();

    const templateSource = await readFile(templatePath ?? "", "utf8");
    const render = createRequire(import.meta.url)("lodash/template") as (
      source: string,
    ) => (values: Record<string, unknown>) => string;
    const desktopEntry = render(templateSource)({
      categories: ["Science", "Utility"],
      description: "Viewer",
      genericName: "Sensor Viewer",
      mimeType: [],
    ...
    const desktopId = packageMetadata.desktopName.replace(/\.desktop$/u, "");

    expect(fields.get("StartupWMClass")).toBe(desktopId);
    expect(fields.get("Icon")).toBe(desktopId);
    expect(fields.get("Exec")?.split(" ")[0]).toBe(desktopId);
  });
...
describe("narrow renderer IPC", () => {
    ...
    expect(new Set(handlers.keys())).toEqual(
      new Set(
        Object.values(IPC_CHANNELS).filter(
          (channel) =>
            channel !== IPC_CHANNELS.event &&
            channel !== IPC_CHANNELS.menuCommand &&
            channel !== IPC_CHANNELS.menuState &&
            channel !== IPC_CHANNELS.windowCommand &&
            channel !== IPC_CHANNELS.windowState,
        ),
      ),
    );
    await expect(
  ...
  it("exposes only a validated desktop platform and bounded window commands", async () => {
    const ipcRenderer: IpcRendererLike = {
    ...
    const api = createNetftApi(ipcRenderer, "darwin");

    expect(api.platform).toBe("darwin");
    await api.performWindowCommand({ type: "toggle-full-screen" });
    await expect(
      api.performWindowCommand({
        type: "open-external",
    ...
    expect(ipcRenderer.invoke).toHaveBeenCalledWith(
      IPC_CHANNELS.windowCommand,
      { type: "toggle-full-screen" },
    );
    expect(api).not.toHaveProperty("ipcRenderer");
  });

  it("publishes bounded menu state and receives menu commands without exposing raw IPC", () => {
    const emitter = new EventEmitter();
    const ipcRenderer: IpcRendererLike = {
      invoke: vi.fn(),
      send: vi.fn(),
    ...
    const unsubscribe = api.subscribeMenuCommands(listener);
    emitter.emit(
      IPC_CHANNELS.menuCommand,
      {},
      { type: "set-time-window", seconds: 30 },
    );
    unsubscribe();

    expect(ipcRenderer.send).toHaveBeenCalledWith(
      IPC_CHANNELS.menuState,
      expect.objectContaining({ connection: "streaming" }),
    );
