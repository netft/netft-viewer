...
describe("ChartWorkspace", () => {
  it("keeps symbol-only layout controls accessible and functional", () => {
    ...
    const combined = screen.getByTestId("chart-mode-combined");
    const panels = screen.getByTestId("chart-mode-panels");

    expect(combined.textContent?.trim()).toBe("");
    expect(panels.textContent?.trim()).toBe("");
    expect(combined.getAttribute("aria-label")?.trim().length).toBeGreaterThan(
      0,
    );
    expect(panels.getAttribute("aria-label")?.trim().length).toBeGreaterThan(0);
    expect(
      combined.querySelector('[data-symbol-style="rounded"]'),
    ).not.toBeNull();
    expect(
      panels.querySelector('[data-symbol-style="rounded"]'),
    ).not.toBeNull();

    fireEvent.click(panels);
    expect(panels.ariaPressed).toBe("true");
  ...
  it("freezes data on pause, clears it on resume, and accepts only new samples", () => {
    const runtime = new TestRuntime();
    let sink: ((event: RendererEvent) => void) | undefined;
    const view = render(
      <ChartWorkspace
    ...
    act(() => {
      sink?.(plotBatch("2000000000"));
    });
    expect(screen.getByTestId("chart-workspace").dataset.pointCount).toBe("6");
  });

  it("clears stale data at connection-generation boundaries", () => {
    const runtime = new TestRuntime();
    const view = render(
      <ChartWorkspace
        runtime={runtime}
  ...
  it("exposes exact windows and keyboard-native axis visibility controls", () => {
    ...
    expect(screen.queryByTestId("chart-reset")).toBeNull();
