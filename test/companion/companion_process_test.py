...
PACKAGE_VERSION = json.loads(
    (SOURCE_DIR / "package.json").read_text(encoding="utf-8")
)["version"]
DEFAULT_COMPANION = (
...
class CompanionProcess:
    ...
    def close(self) -> None:
        ...
        if shutdown_timeout is not None:
            raise AssertionError(
                "companion did not stop during test cleanup"
...
class FakeSensor:
    def __init__(self) -> None:
        ...
        self.http = LoopbackHTTPServer(("127.0.0.1", 0), Handler)
...
class CompanionProcessTest(unittest.TestCase):
    def test_hello_reports_matching_versions(self) -> None:
        ...
        try:
            ...
            self.assertEqual(event["payload"]["appVersion"], PACKAGE_VERSION)
        finally:
            companion.close()
