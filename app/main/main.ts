...
} from "./window-command-handler";
import { buildViewerWindowOptions } from "./window-options";
...
export interface CreateViewerWindowOptions {
  ...
  beforeLoad?: (window: BrowserWindowLike) => void;
  iconPath?: string;
...
export const createViewerWindow = async (
  ...
  if (options.iconPath !== undefined && !isAbsolute(options.iconPath)) {
...
const boot = async (): Promise<void> => {
  ...
  const window = await createViewerWindow({
    ...
    beforeLoad: (createdWindow) => {
      cleanupMenu = installApplicationMenu({
        ipcMain,
        menu: Menu,
        ...
        trustedWebContents: createdWindow.webContents,
      });
    },
  ...
  const cleanupWindowCommands = registerWindowCommandHandler({
    app,
    ipcMain,
    openExternal: async (url) => shell.openExternal(url),
    trustedWebContents: window.webContents,
  ...
  const cleanupWindowState = registerWindowStateBridge({
    trustedWebContents: window.webContents,
    window,
  });
