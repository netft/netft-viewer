...
const requireRegularArtifact = async (root, path) =>
  requireRegularPackageFile(root, relative(root, path));
...
export const verifyPackageArtifacts = async ({
  ...
  await Promise.all(
    artifacts.map(({ path }) => requireRegularArtifact(outDirectory, path)),
  );
